void trig_updated(){
    ...
    TString directory = "/tmp/zdemirag/flat_Nov17/";
    TString directory_data = "/tmp/zdemirag/flat_Nov17/";


    TFile *f = new TFile(directory+"WJetsToLNu_HT-200To400_TuneCUETP8M1_13TeV-madgraphMLM-pythia8+RunIISpring15DR74-Asympt25ns_MCRUN2_74_V9-v1+AODSIM.root","READ");
    ...
    if (var == "recoil"){
        name = "met";
        offline = "jet1Pt>100. && TMath::Abs(jet1Eta)<2.5 && jet1isMonoJetId==1 && n_tightlep > 0 && n_looselep>0 && abs(minJetMetDPhi)>0.5 && lep1Pt>40.";
    }

    else {
        name = "jet1Pt";
        offline = "TMath::Abs(jet1Eta)<2.5 && jet1isMonoJetId==1 && n_tightlep > 0 && n_looselep>0 && abs(minJetMetDPhi)>0.5 && met > 250. && lep1Pt>40.";
    }
    ...
    //Electron
    TString cut_den = "(triggerFired[3]) && "+offline;
    TString cut_num = "(triggerFired[3]) && (triggerFired[1] || triggerFired[0] || triggerFired[2]) && "+offline;

    events->Draw(name+">>den_mc_met",cut_den,"");
    events->Draw(name+">>num_mc_met",cut_num,"");

    TFile *f2 = new TFile(directory_data+"SingleElectron+Run2015D-PromptReco-v4+AOD.root","READ");
    //TFile *f2 = new TFile(directory_data+"SingleMuon+Run2015D-PromptReco-v4+AOD.root","READ");
    ...
    latex1->DrawLatex(0.9, 0.94,"2.1 fb^{-1} (13 TeV)");
    ...
    if (var == "recoil"){
        ...
        dummy2->GetXaxis()->SetTitle("Leading Jet p_{T} [GeV]");
    }
    ...
    c1->SaveAs("/afs/cern.ch/user/z/zdemirag/www/Monojet/topupv3/signal/trigeff_"+var+".pdf");
    c1->SaveAs("/afs/cern.ch/user/z/zdemirag/www/Monojet/topupv3/signal/trigeff_"+var+".png");
    c1->SaveAs("/afs/cern.ch/user/z/zdemirag/www/Monojet/topupv3/signal/trigeff_"+var+".C");
    c1->SaveAs("/afs/cern.ch/user/z/zdemirag/www/Monojet/topupv3/signal/trigeff_"+var+".root");
