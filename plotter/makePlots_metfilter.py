...
from LoadData_metfilter import *
...
def plot_stack(channel, name,var, bin, low, high, ylabel, xlabel, setLog = False):
    ...
    folder = '/afs/cern.ch/user/z/zdemirag/www/misc/'
    ...
    #    binLowE = [200,250,300,350,400,500,600,1000,1500,3000]
    #    added = TH1D('added','added',9,array('d',binLowE))
    #else:
    ...
    for Type in reordered_physics_processes:
        ...
        #    Variables[Type] = TH1F(histName,histName,9,array('d',binLowE))
        #else:
        ...
        if Type is not 'data' and Type is not 'data2' and Type is not 'data3' :
            print "What is your Type: ", Type
...
#"(1.00708119847*(met>200.0&&met<=250.0)+0.988524553689*(met>250.0&&met<=300.0)+1.10261568311*(met>300.0&&met<=350.0)+0.973358572308*(met>350.0&&met<=400.0)+0.946123092851*(met>400.0&&met<=500.0)+0.955509178272*(met>500.0&&met<=600.0)+2.33655856035*(met>600.0&&met<=1000.0))"
            g_postfit = "(0.974144987372*(met>200.0&&met<=250.0)+0.957144152837*(met>250.0&&met<=300.0)+0.924837461929*(met>300.0&&met<=350.0)+0.907693573019*(met>350.0&&met<=400.0)+0.897316971506*(met>400.0&&met<=500.0)+0.912633534821*(met>500.0&&met<=600.0)+0.877052889306*(met>600.0&&met<=1000.0))"
        else:
            wm_postfit = "(1.0)"
            zm_postfit = "(1.0)"
            g_postfit  = "(1.0)"
        ...
        if Type is not 'data' and Type is not 'signal_dm' and Type is not 'signal_dm_av_50_2' and Type is not 'signal_dm_av_1_2' and Type is not 'signal_dm_av_1_100' and Type is not 'signal_dm_av_10_100' and Type is not 'signal_dm_s_500_10' and Type is not 'signal_dm_s_500_1' and Type is not 'signal_dm_s_1_300' and Type is not 'signal_dm_s_50_300' and Type is not 'signal_h_vbf' and Type is not 'signal_h_ggf' and Type is not 'data2' and Type is not 'data3':
            ...
            if Type.startswith('GJets') :
                ...
                #    makeTrees(Type,'events',channel).Draw(var + " >> " + histName,"(" + cut_standard + " && met < 1000.) *mcWeight*" +str(w)+"*"+str(w_trig),"goff") 
                #else:
        ...
        if Type.startswith('data'):
            ...
            if channel is 'signal' or channel is 'Zmm' or channel is 'Wmn':
                if Type is 'data3':
                    makeTrees(Type,"events",channel).Draw(var + " >> " + histName, "(" + cut_standard + " && (triggerFired[0]==1 || triggerFired[1]==1 || triggerFired[2]==1) && jet1isMonoJetIdNew==1 )", "goff")
                else:
        ...
        #if Type.startswith("Zvv"):
        #    nbin = Variables[Type].GetNbinsX()
        #    for b in range(0,nbin+1):
        #        print b+1 , Type, Variables[Type].Integral(b,b+1)
        #    print "overflow", Type, Variables[Type].Integral(0,nbin+1)
    ...
    for process in  ordered_physics_processes:
        ...
        if physics_processes[process]['label'] != lastAdded:
            ...
            if process is not 'data' and process is not 'signal_dm' and process is not 'signal_dm_av_1_2' and process is not 'signal_h_ggf' and process is not 'signal_h_vbf' and process is not 'data2' and process is not 'data3':
    ...
    if setLog:
        c4.SetLogy()
        #stack.SetMaximum( stack.GetMaximum()  +  1e4*stack.GetMaximum() )
    ...
    #Variables['data'].SetMarkerStyle(21)
    #Variables['data'].SetMarkerColor(2);
    #Variables['data' ].Draw("Esame")


    Variables['data2'].SetMarkerStyle(4)
    Variables['data2'].Draw("Esame")

    Variables['data3'].Draw("Esame")
    ...
    data = Variables['data3'].Clone()

    #plot_ratio(False,data,added,bin,xlabel,0.0,2.0,5)
    ...
    if postfit is True:
        c4.SaveAs(folder+'Histo_Postfit' + name + '_'+channel+'.pdf')
        c4.SaveAs(folder+'Histo_Postfit' + name + '_'+channel+'.png')
        c4.SaveAs(folder+'Histo_Postfit' + name + '_'+channel+'.C')
    else:
        if ht_bin:
            c4.SaveAs(folder+'/Histo_lov3_bin'+str(metcut)+'_' + name + '_'+channel+'.pdf')
            c4.SaveAs(folder+'/Histo_lov3_bin'+str(metcut)+'_' + name + '_'+channel+'.png')
            c4.SaveAs(folder+'/Histo_lov3_bin'+str(metcut)+'_' + name + '_'+channel+'.C')
        else:
            ...
            #c4.SaveAs(folder+'/Histo_lov5_' + name + '_'+channel+'.pdf')
            #c4.SaveAs(folder+'/Histo_lov5_' + name + '_'+channel+'.png')
            #c4.SaveAs(folder+'/Histo_lov5' + name + '_'+channel+'.C')
...
arguments['met']    = ['met','met',30,200,2000,'Events/60 GeV','E_{T}^{miss} [GeV]',True]
arguments['jetpt']  = ['jetpt','jet1Pt',18,100,1000,'Events/50 GeV','Leading Jet P_{T} [GeV]',True]
arguments['jet2pt']  = ['jet2pt','jet2Pt',18,100,1000,'Events/50 GeV','Leading Jet P_{T} [GeV]',True]
